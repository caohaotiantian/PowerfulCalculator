...
def plot():
    try:
        ...
        return jsonify(
            {'expression': str(ps), 'latex': latex(ps), 'img': 'data:image/png;base64,' + str(encoded_img)[2:-1]})
    ...
    try:
        ...
        if U.shape[1] == 3:
            ...
            for i in range(U.shape[0]):
                ...
                ax.add_artist(
                    Arrow3D([0, x / magn], [0, y / magn], [0, z / magn], mutation_scale=15, lw=3, arrowstyle="-|>",
                            color="purple"))
                ax.add_artist(Arrow3D([0, x / magn], [0, y / magn], [0, 0], mutation_scale=8, lw=2, arrowstyle="-|>",
...
def mplot():
    try:
        ...
        var.sort()
        if len(ps.free_symbols) > 0:
            raise ValueError('绘制矩阵图需要标量输入')
        if not 'Matrix' in str(type(ps)):
            raise ValueError('绘制矩阵图需要矩阵输入')
...
def graph():
    try:
        ...
        var.sort()
        if len(ps.free_symbols) > 0:
            raise ValueError('输入应为标量值')
        if not 'Matrix' in str(type(ps)):
            raise ValueError('输入应为矩阵')
        if ps.shape[0] != ps.shape[1]:
            raise ValueError('输入应为方阵')
...
def la_eigen():
    try:
        ...
        return jsonify(
            {'in': latex(ps), 'vectors': latex([v for v in ps.eigenvects()]), 'values': latex(ps.eigenvals())})
...
def graph_mst():
    try:
        ...
        return jsonify({'in': latex(ps), 'out': latex(mst), 'out_expression': str(mst), 'weight': str(float(weight)),
                        'image': encoded_img})
