...
def is_simple(M):
    return all([m == 0 for m in np.diagonal(M)]) and is_binary(M)


def is_binary(M):
    return all([M[i, j] == 0 or M[i, j] == 1 for j in range(M.shape[1]) for i in range(M.shape[0])])
...
def is_directed(M):
    return not is_symmetric(M)


def is_weighted(M):
    return not is_binary(M)


def degree_matrix(G):
    return np.sum(np.vectorize(not_zero)(G), axis=0) * np.eye(G.shape[0])
...
def plot(G):
    if is_symmetric(G).all():
        return plot_graph(G)
    else:
        return plot_digraph(G)
